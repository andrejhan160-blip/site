...
export function UserMenu({ name, email, role }: { name: string; email: string; role: Role | 'CLIENT' }) {
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm transition-colors hover:bg-[var(--color-surface-muted)]">
        ...
        <span className="hidden text-left sm:block">
          ...
          <span className="block text-[11px] text-[var(--color-ink-subtle)]">{ROLE_LABELS[role]}</span>
        </span>
